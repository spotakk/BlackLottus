...
function runPlate(source,vehPlate)
	...
	if userPlate then
		...
		vRPC.playSound(source,"Event_Message_Purple","GTAO_FM_Events_Soundset")
		TriggerClientEvent("Notify",source,"important","Atenção","<b>Passaporte:</b> "..identity["id"].."<br><b>Nome:</b> "..identity["name"].." "..identity["name2"].."<br><b>Nº:</b> "..identity["phone"],"amarelo",10000)
	else
		...
		vRPC.playSound(source,"Event_Message_Purple","GTAO_FM_Events_Soundset")
		TriggerClientEvent("Notify",source,"important","Atenção","<b>Passaporte:</b> 9.999<br><b>Nome:</b> "..plateSave[vehPlate][1].."<br><b>Nº:</b> "..plateSave[vehPlate][2],"amarelo",10000)
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- POLICE:RUNARREST
				...
				if userPlate then
					...
					if inVehicle[1] then
						if inVehicle[1]["arrest"] <= os.time() then
							...
							TriggerClientEvent("Notify",source,"check","Sucesso","Veículo apreendido.","verde",5000)
						else
							TriggerClientEvent("Notify",source,"important","Atenção","Veículo já se encontra apreendido.","amarelo",5000)
						end
					end
				end
			end
