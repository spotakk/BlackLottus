...
-----------------------------------------------------------------------------------------------------------------------------------------
local initLocates = {
	{ 371.61,-1612.23,29.28 }, --impound 1
 	{ 1737.6,3709.81,34.14 }, --impound 2
 	{ -274.6,6120.98,31.32 } --impound 3
}
-----------------------------------------------------------------------------------------------------------------------------------------
-- DESPAWNLOCATES
-----------------------------------------------------------------------------------------------------------------------------------------
local despawnLocates = {
	{ 403.1,-1633.41,29.28 }, --1
	{ 1724.84,3715.31,34.22 }, --2
	{ -305.45,6117.62,31.49 } --3
}
-----------------------------------------------------------------------------------------------------------------------------------------
-- VEHMODELS
-----------------------------------------------------------------------------------------------------------------------------------------
			...
			if vehDistance <= 15 then
				if inTowed then
					...
					if vehicle ~= vehTowed then
						...
						TriggerEvent("initRouter",despawnLocates[serviceLocate][1],despawnLocates[serviceLocate][2],despawnLocates[serviceLocate][3])
						TriggerEvent("Notify","important","Atenção","Leve o veiculo ao local marcado no GPS.","amarelo",5000)
					end
				end
			else
...
RegisterNetEvent("initRouter",function(...)
	 ...
	 AddTextComponentString('Patio de veiculos')
	 EndTextCommandSetBlipName(blip)
end)


RegisterNetEvent("finishRouter",function()
	RemoveBlip(blip)
end)