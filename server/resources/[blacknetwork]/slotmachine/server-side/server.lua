...
AddEventHandler("slotmachine:taskStartSlots",function(index,data)
	...
	local user_id = vRP.getUserId(source)
	if user_id then
			if vRP.tryGetInventoryItem(user_id,"chips",150,true) then
...
function cRP.checkGrana()
	local source = source
	local user_id = vRP.getUserId(source) 
	if user_id then
		local consultItem = vRP.getInventoryItemAmount(user_id,"chips")
		if consultItem[1] >= 150  then
			return true
		else 
			return false
		end				
...
function CheckForWin(w,data)
	...
	if user_id then
		...
		if total > 0 then
			vRP.giveInventoryItem(user_id,"chips",total,true)
		end
	end
end