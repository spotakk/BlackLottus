...
local bedsIn = {
	["Santos"] = {
		{ -460.29,-288.67,35.82,22.68 },
		{ -454.91,-286.48,35.82,22.68 },
		{ -459.0,-279.65,35.82,204.1 },
		{ -462.75,-281.23,35.82,204.1 }
	},
	["Sandy"] = {
...
-- THREADCHECKIN
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("checkin:initCheckHospital")
AddEventHandler("checkin:initCheckHospital",function(Hospital)
	
	local ped = PlayerPedId()
	if vSERVER.checkParamedicServices() then
		for _,v in pairs(bedsIn[Hospital]) do
			local checkPos = nearestPlayer(v[1],v[2],v[3])
			if not checkPos then
				if vSERVER.paymentCheckin() then
