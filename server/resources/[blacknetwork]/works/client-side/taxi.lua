...
RegisterNetEvent("work:setTraje:taxi",function(k)
	local model = vSERVER.modelPlayer()
	if model == "mp_m_freemode_01" or "mp_f_freemode_01" then
		TriggerEvent("updateRoupas", TaxiTraje[model])
	end
end)

RegisterNetEvent("init:target:taxi", function(k)
	if serviceStatus ~= false then
		TriggerEvent("Notify", "important", "Atenção", "Voce ainda não finalizou o emprego de <b>Taxi</b>","amarelo",5000)
		return
	end
	if serviceStatus then
		...
		if vSERVER.initService(true) then
			TriggerEvent("Notify", "important", "Atenção", "Voce acabou de iniciar o emprego de <b>Taxi</b>","amarelo", 5000)
			locateSelect = k
...
CreateThread(function()
	for k, v in pairs(initService) do
		exports["target"]:AddCircleZone("works" .. k, vec3(v[1], v[2], v[3]), 0.5, {
			name = "works",
			heading = 3374176
		}, {
			shop = k,
			...
			options = {
				{
					event = "init:target:taxi",
					label = "Iniciar",
					tunnel = "client"
				},
				{
					event = "work:setTraje:taxi",
					label = "Traje",
					tunnel = "client"
				}
			}
		...
		if IsPedInAnyVehicle(ped) then
			if serviceStatus then
				...
				if distance <= 100 then
					...
					DrawMarker(1, stopVehicle[locateSelect][selectPosition][1], stopVehicle[locateSelect][selectPosition][2],
						stopVehicle[locateSelect][selectPosition][3] - 3, 0, 0, 0, 0, 0, 0, 5.0, 5.0, 3.0, 255, 255, 255, 25, 0, 0, 0,
						0)
					DrawMarker(21, stopVehicle[locateSelect][selectPosition][1], stopVehicle[locateSelect][selectPosition][2],
						stopVehicle[locateSelect][selectPosition][3], 0, 0, 0, 0, 180.0, 130.0, 1.5, 1.5, 1.0, 46, 110, 76, 100, 0, 0,
						0, 1)
...
function generatePassenger(vehicle)
	...
	local myObject, objNet = vRPS.CreatePed(spawnModels[randModels], spawnPeds[locateSelect][selectPosition][1],
		spawnPeds[locateSelect][selectPosition][2], spawnPeds[locateSelect][selectPosition][3], 3374176, 4)
	if myObject then
		Wait(1000)
...
function blipPassenger()
	...
	serviceBlip = AddBlipForCoord(stopVehicle[locateSelect][selectPosition][1],
		stopVehicle[locateSelect][selectPosition][2], stopVehicle[locateSelect][selectPosition][3])
