	...
	RegisterCommand("globalFunctions",function(source,args,rawCommand)
		if not LocalPlayer["state"]["Commands"] and not LocalPlayer["state"]["Handcuff"] and not menuOpen and MumbleIsConnected() then
			...
			if GetEntityHealth(ped) > 101 then
				...
				exports["dynamic"]:AddButton("Aplicar","Vestir as roensure ensure as.","player:outfitFunctions","preaplicar","premiumfit",true)
			...
			if not IsPedInAnyVehicle(ped) then
				...
				exports["dynamic"]:AddButton("Banco Dianteiro Esquerdo","Sentar no banco do motorista.","player:seatPlayer","0","vehicle",false)
					...
					exports["dynamic"]:AddButton("Banco Extra Esquerdo","Sentar no banco do passageiro.","player:seatPlayer","4","vehicle",false)
					exports["dynamic"]:AddButton("Banco Extra Direito","Sentar no banco do passageiro.","player:seatPlayer","5","vehicle",false)
			...
			exports["dynamic"]:AddButton("Propriedades","Ativa/Desativa as propriedades no mapa.","propertys:Blips","","propertys",false)
		...
		if not LocalPlayer["state"]["Commands"] and not LocalPlayer["state"]["Handcuff"] and not menuOpen and MumbleIsConnected() then
			...
			if GetEntityHealth(ped) > 101 then
				...
				if LocalPlayer["state"]["Police"] then
					...
					exports["dynamic"]:AddButton("Soldado","Fardamento de Soldado","player:presetFunctions","2","prePolice",true)
					exports["dynamic"]:AddButton("SPEED","Fardamento de SPEED","player:presetFunctions","3","prePolice",true)
					exports["dynamic"]:AddButton("GTM","Fardamento de GTM","player:presetFunctions","4","prePolice",true)
					exports["dynamic"]:AddButton("CORE","Fardamento de CORE","player:presetFunctions","5","prePolice",true)			
			...
			--		exports["dynamic"]:AddButton("Computador","Computador de bordo policial.","police:openSystem","",false,false)
				elseif LocalPlayer["state"]["Paramedic"] then
					exports["dynamic"]:AddButton("Residente","Fardamento de Aluno.","player:presetFunctions","9","preMedic",true)
					exports["dynamic"]:AddButton("Enfermeiro","Fardamento de Enfermeiro.","player:presetFunctions","10","preMedic",true)
					exports["dynamic"]:AddButton("Diretor","Fardamento de Diretor.","player:presetFunctions","11","preMedic",true)
