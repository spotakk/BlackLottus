	...
	window.addEventListener("message",function(event){
		switch(event.data.action){
			case "showMenu":
				...
				renderHTML()
				requestChest();
				setTimeout(() => {$(".inventory").css("display","flex");}, 200);
			break;
			...
			case "updateWeight":
				$("#weightTextLeft").html(`${(event["data"]["invPeso"]).toFixed(2)} / ${(event["data"]["invMaxpeso"]).toFixed(2) + "KG"}`);
				$("#weightTextRight").html(`${(event["data"]["chestPeso"]).toFixed(2)} / ${(event["data"]["chestMaxpeso"]).toFixed(2) + "KG"}`);

				$("#weightBarLeft").html(`<div class="weightBar" style="width: ${event["data"]["invPeso"] / event["data"]["invMaxpeso"] * 100}%"></div>`);
				$("#weightBarRight").html(`<div class="weightBar" style="width: ${event["data"]["chestPeso"] / event["data"]["chestMaxpeso"] * 100}%"></div>`);
			break;
		}
	});
	...
	$('.empty').droppable({
		...
		drop: function(event,ui){
			...
			if (origin === "invLeft" && tInv === "invLeft"){
				...
				$.post("http://homes/Take",JSON.stringify({
					item: itemData.key,
					slot: itemData.slot,
					target: target,
					amount: parseInt(amount)
				...
				$.post("http://homes/Store",JSON.stringify({
					item: itemData.key,
					slot: itemData.slot,
					target: target,
					amount: parseInt(amount)
				...
				$.post("http://homes/Update",JSON.stringify({
					item: itemData.key,
					slot: itemData.slot,
					target: target,
					amount: parseInt(amount)
...
const requestChest = () => {
	$.post("http://homes/Request",JSON.stringify({}),(data) => {
		$("#weightTextLeft").html(`${(data["invPeso"]).toFixed(2) } / ${(data["invMaxpeso"]).toFixed(2) + "KG"}`);
		$("#weightTextRight").html(`${(data["chestPeso"]).toFixed(2)} / ${(data["chestMaxpeso"]).toFixed(2) + "KG"}`);
		$("#weightBarLeft").html(`
			<div class="weightBar" style="width: ${data["invPeso"] / data["invMaxpeso"] * 100}%"></div>
		`);
		$("#weightBarRight").html(`
			<div class="weightBar" style="width: ${data["chestPeso"] / data["chestMaxpeso"] * 100}%"></div>
		`);
		...
		for (let x = 1; x <= data["invMaxpeso"]; x++){
			...
			if (data.myInventory[slot] !== undefined){
				...
				const item = `<div class="item populated" title="" data-max="${v["max"]}" data-type="${v["type"]}" data-description="${v["desc"]}" style="background-image: url('nui://inventory/web-side/images/${v.index}.png'); background-position: center; background-repeat: no-repeat;" data-amount="${v.amount}" data-peso="${v.peso}" data-item-key="${v.key}" data-name-key="${v.name}" data-slot="${slot}">
					...
					<div class="durability" style="width: ${actualPercent == 1 ? "100":actualPercent}%; background: ${actualPercent == 1 ? "#FF0053":colorPicker(actualPercent)};"></div>
				...
				<div class="left_top">	
					...
					<div class="weight_player">
						<div class="player_info">
							...
							<span>${userName} <span>#${userId}</span></span>
						</div>
						<div class="weight">
