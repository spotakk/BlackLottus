...
AddEventHandler("vRP:playerActive",function(user_id,name)
	...
	SetRichPresenceDiscord(user_id,name)
	...
	-- ReplaceHudColourWithRgba(0, 0, 0, 255)
...
function SetRichPresenceDiscord(user_id, name)
    local LabelDiscord = "#"..user_id.." "..name
    if name == 'Individuo Indigente' then
        LabelDiscord = "#"..user_id.." Criando personagem"
    end
    SetDiscordAppId(1232452204540203119)
    SetDiscordRichPresenceAsset("11")
    SetRichPresence(LabelDiscord)
    SetDiscordRichPresenceAssetSmall("22")
    SetDiscordRichPresenceAssetText("Black Network - Exclusive")
    SetDiscordRichPresenceAssetSmallText("Black Network - Exclusive")
    SetDiscordRichPresenceAction(0,"Black Network","https://discord.gg/5qdX6KkrdC")
    SetDiscordRichPresenceAction(1,"Youtube","https://www.youtube.com/watch?v=lDFBlTpcYdY&t=1613s")

    while true do
        Wait(1000)

        local identity = vRPS.felpGetIdentity(user_id)
		local newName = identity["name"].." "..identity["name2"]
        if newName ~= name or newName ~= 'Individuo Indigente' then 
            name = newName 
            LabelDiscord = "#"..user_id.." "..name
            if name == 'Individuo Indigente' then
                LabelDiscord = "#"..user_id.." Criando personagem"
            end
            SetRichPresence(LabelDiscord) 
