...
local MenuAnimationIsOpen = false
...
RegisterCommand("cRbind",function(source,args,rawCommand)
	if GetGameTimer() >= cdBtns and LocalPlayer["state"]["Active"] and MumbleIsConnected() and not MenuAnimationIsOpen then
		cdBtns = GetGameTimer() + 1000

		local ped = PlayerPedId()
		if not IsPauseMenuActive() and not LocalPlayer["state"]["Buttons"] and not LocalPlayer['state']['onMenu'] and not LocalPlayer["state"]["Commands"] and not LocalPlayer["state"]["Phone"] and GetEntityHealth(ped) > 100 and not LocalPlayer["state"]["Cancel"] and not IsPedReloading(ped) then
			if parseInt(args[1]) >= 1 and parseInt(args[1]) <= 5 and GetEntityHealth(PlayerPedId()) > 101 then
...
-- PROMPT
-----------------------------------------------------------------------------------------------------------------------------------------
function tvRP.prompt(title,text,value)
	SendNUIMessage({ act = "prompt", title = title, text = text, value = value})
	SetNuiFocus(true,true)
end

RegisterNetEvent("updateMenuIsOpen")
AddEventHandler("updateMenuIsOpen",function(status)
	MenuAnimationIsOpen = status
end)