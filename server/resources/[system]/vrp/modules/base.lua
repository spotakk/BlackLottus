...
-- DISCORDHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webHook = "https://discord.com/api/webhooks/1053747870794260570/8BYoh3YjrLRXORhhA2g3PJD_1uhaJWcpI_P8-WWRd-5n85FleKT1c2iU9a5L37LtTSg2"
-----------------------------------------------------------------------------------------------------------------------------------------
-- MYSQL
-----------------------------------------------------------------------------------------------------------------------------------------
local mysqlDriver
...
--[[ 			dataTable["position"] = { x = mathLegth(coords["x"]), y = mathLegth(coords["y"]), z = mathLegth(coords["z"]) } ]]
			...
			vRP.execute("playerdata/setUserdata", { user_id = user_id, key = "Position", value = json.encode({ x = mathLegth(coords["x"]), y = mathLegth(coords["y"]), z = mathLegth(coords["z"]) })})
...
function vRP.characterChosen(source,user_id,model,locate)
	...
	if model ~= nil then
		...
		vRP.generateItem(user_id,"dollars",750,false)
		...
		if locate == "Sul" then
			vRP.execute("playerdata/setUserdata", { user_id = user_id, key = "Position", value = json.encode({ x = -28.08, y = -145.96, z = 56.99 })})
			--[[ vRP.userTables[user_id]["position"] = { x = -28.08, y = -145.96, z = 56.99 } ]]
		else
			vRP.execute("playerdata/setUserdata", { user_id = user_id, key = "Position", value = json.encode({ x = 1935.59, y = 3721.93, z = 32.87 })})
