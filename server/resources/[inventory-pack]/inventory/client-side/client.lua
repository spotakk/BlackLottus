...
AddEventHandler("inventory:Buttons",function(status)
	...
	LocalPlayer["state"]["disablePhone"] = status
end)
...
local adrenalineCds = {
	{ 318.65,-557.94,28.75 },
	{ 1550.97,3517.93,35.99 },
	{ 283.79,6787.24,15.69 }
...
RegisterCommand("openBackpack",function(source,args,rawCommand)
    ...
    local pid = PlayerId()
    local vehDoors = GetVehicleDoorLockStatus(pedVeh)
    if GetEntityHealth(ped) > 101 and not LocalPlayer["state"]["Buttons"] and MumbleIsConnected() then
        if pedVeh ~= 0 and vehDoors ~= 2 then
            TriggerServerEvent("trunkchest:openTrunk")
            return
        end
        if not LocalPlayer["state"]["Commands"] and not LocalPlayer["state"]["Handcuff"] and not IsPlayerFreeAiming(pid) then
            Backpack = true
...
local weaponAttachs = {
	...
	["attachsMagazine"] = {
		...
		["WEAPON_BARRET"] = "COMPONENT_BARRET_CLIP_01",
...
local blackListAnimWeapons = {
	['WEAPON_SWITCHBLADE'] = true
}
function weaponCanPlayAnim(weaponName)
	return not blackListAnimWeapons[weaponName]
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- PUTWEAPONHANDS
-----------------------------------------------------------------------------------------------------------------------------------------
...
Citizen.CreateThread(function()
	while true do
		...
		for _,v in pairs(Drops) do
			...
			if distance <= 50 then
				timeDistance = 1
				DrawMarker(21,v["coords"][1],v["coords"][2],v["coords"][3] + 0.25,0.0,0.0,0.0,0.0,180.0,0.0,0.25,0.35,0.25,136,36,249,100,1,0,0,1)
			end
		end
...
RegisterNUICallback("requestInventory",function(data,cb)
	...
	local inventario,invPeso,invMaxpeso,playerName,playerId = vSERVER.requestInventory()
	if inventario then
		cb({ inventario = inventario, drop = Items, invPeso = invPeso, invMaxpeso = invMaxpeso,name = playerName,id = playerId })
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- PICKUPITEM
