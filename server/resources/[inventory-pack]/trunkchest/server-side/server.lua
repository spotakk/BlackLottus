	...
	if user_id then
		...
		if vehicle then
			...
			if userPlate then
				...
				return myInventory,myChest,vRP.inventoryWeight(user_id),vRP.getWeight(user_id),vRP.chestWeight(result),parseInt(vehWeight[user_id]),identity["name"].." "..identity["name2"],user_id
			end
		end
	end
end
