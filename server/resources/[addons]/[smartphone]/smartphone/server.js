  ...
  (pz.deleteManyImages = (...e) => {
    ...
    if (f.length > 0) {
      ...
      return (
        ...
        pz.put("https://fivem-static.jesteriruka.dev/storage/deleteMany", h, i)
      );
    }
...
const pF = {
    ...
    fetchTables() {
      return new Promise(async (c) => {
        ...
        if (
          pF.tables.includes("black_characters") ||
          pF.hasTable("black_dummy")
        ) {
    ...
    get userDataTable() {
      return pF.firstTable("vrp_user_data", "zuser_data", "black_playerdata");
    },
    ...
    async getIdentitiesBy(c = "user_id", d = []) {
      ...
      if (pF.hasTable("nyo_character"))
        ...
        if (pF.hasTable("drip_characters"))
          ...
          if (pF.hasTable("black_characters"))
            (c = c == "user_id" ? "id" : c),
              (e = await pc("black_characters")
                .whereIn(c, d)
                .select("id", "name", "name2", "phone"));
          else {
            if (pF.hasTable("black_dummy")) {
              (c = c == "user_id" ? "id" : c),
...
p6(async function () {
  try {
    ...
    console.log('^2SMARTPHONE INICIADO^0');
...
const qc = {
  deepweb: "http://127.0.0.1/smartphone/apps/tor.png",
  instagram: "http://127.0.0.1/smartphone/apps/instagram.png",
  bank: "http://127.0.0.1/smartphone/apps/bank.png",
  paypal: "http://127.0.0.1/smartphone/apps/paypal.png",
  olx: "http://127.0.0.1/smartphone/apps/olx.png",
  twitter: "http://127.0.0.1/smartphone/apps/twitter.png",
  services: "http://127.0.0.1/smartphone/apps/config.png",
  weazel: "http://127.0.0.1/smartphone/apps/weazel.png",
  casino: "http://127.0.0.1/smartphone/apps/blaze.png",
};
let qd = qc,
  qe = 0;
...
pF.ready(async (c) => {
  ...
  if (
    (await pF.hasTable("wise_multas")) &&
    !(await pF.firstColumn("wise_multas", "id", "multa_id"))
  ) {
    pc.query(
      "ALTER TABLE wise_multas ADD COLUMN id BIGINT PRIMARY KEY AUTO_INCREMENT"
    );
  }
});

async function qx(c) {
  const d = pF.firstTable("black_characters", "characters"),
...
async function qy(d, e) {
  ...
  if (f) {
    ...
    if (pF.hasTable("black_bank")) {
      const k = await pF.firstColumn("black_bank", "user_id", "Passport"),
        l = await pF.firstColumn("black_bank", "value", "dvalue"),
        m = {
          mode: "private",
        };
      m[k] = d;
      ...
      if (pF.hasColumn("black_bank", "owner")) {
        n.owner = 1;
      }

      return pc("black_bank").update().where(n).increment(l, e);
    } else {
      for (const r of g) {
...
pF.ready(async () => {
  ...
  pF.ready(async () => {
    ...
    for (let [h, i] of Object.entries(g)) {
      e.push({
        ...
        async deleteOne(j) {
          return await pp.delFines(j, await pp.getFines(j));
        },
      });
    }
  ...
  (pL.bank_extract = async (g) => {
    const h = await pp.getUserId(g),
      i = pF.firstTable("vrp_banco", "vrp_banco", "smartphone_bank");
...
pF.ready(async (c) => {
  ...
  (pL.twitter_register = async (j, k) => {
    ...
    if (u) return v;
      ...
      (k.avatarURL = "http://191.232.191.131/stock/twitter_egg.png"),
