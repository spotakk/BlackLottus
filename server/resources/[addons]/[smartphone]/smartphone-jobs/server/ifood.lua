...
vRP.prepare('ifood/insert', [[INSERT INTO smartphone_ifood_orders (id, user_id, worker_id, store_id, total, fee, created_at) VALUES (@id, @user_id, @worker_id, @store_id, @total, @fee, UNIX_TIMESTAMP())]])
...
AddEventHandler('playerDropped', function()
    ...
    if order then
        ...
        emitNet('smartphone:pusher', order.customer.source, 'BANK_NOTIFY', {
            ...
            subtitle = 'Seu pagamento para NRP*iFood foi estornado'
        })
        cancelOrder(order, true)
    else
        local order = getOrder(source)
...
AddEventHandler('smartphone:isReady', function()
    exports.smartphone:createApp('ifood', 'iFood', 'http://127.0.0.1/smartphone/apps/ifood.webp', 'nui://smartphone-jobs/build/index.html#/ifood')
end)
