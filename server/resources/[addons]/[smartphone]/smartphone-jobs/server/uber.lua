...
vRP.prepare('uber/insert', [[INSERT INTO smartphone_uber_trips (`id`, `user_id`, `driver_id`, `total`, `from`, `to`, `created_at`, `finished_at`) VALUES (@id, @user_id, @driver_id, @total, @from, @to, @created_at, UNIX_TIMESTAMP())]])
...
expose('uber_create_trip', function(source, pos, distance)
    ...
    local trip = {
        ...
        customer = {
            ...
            avatarURL = 'http://127.0.0.1/smartphone/stock/user_square.png',
        },
        push = function(self, ...)
...
expose('uber_accept_trip', function(source, id)
    ...
    notify(trip.customer.source, 'uber', 'Motorista encontrado', string.format('%s (%s) está a caminho', trip.driver.name, trip.driver.plate))
...
AddEventHandler('smartphone:isReady', function()
    exports.smartphone:createApp('uber', 'Uber', 'http://127.0.0.1/smartphone/apps/uber.webp', 'nui://smartphone-jobs/build/index.html#/uber')
end)
