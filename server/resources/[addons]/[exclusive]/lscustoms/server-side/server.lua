...
function cRP.checkPermission(hasPerm)
	...
	if user_id then
		if vRP.getFines(user_id) > 0 then
			TriggerClientEvent("Notify",source,"amarelo","Pendencias","Multas pendentes encontradas.",3000)
			return false
		end
...
AddEventHandler("lscustoms:updateVehicle",function(mods,vehPlate,vehName)
	...
	if userPlate then
		vRP.execute("entitydata/setData",{ dkey = "custom:"..userPlate["user_id"]..":"..userPlate["vehicle"], value = json.encode(mods) })
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- INVEHICLE
