...
$(document).ready(function () {
  window.addEventListener("message", function (event) {
    switch (event.data.action) {
      ...
      case "updateWeight":
        $("#weightTextLeft").html(
          `${event["data"]["invPeso"].toFixed(2)} / ${event["data"][
            "invMaxpeso"
          ].toFixed(2) + "KG"}`
        );
        $("#weightTextRight").html(
          `${event["data"]["chestPeso"].toFixed(2)} / ${event["data"][
            "chestMaxpeso"
          ].toFixed(2) + "KG"}`
        );

        $("#weightBarLeft").html(
          `<div class="weightBar" style="width: ${
            (event["data"]["invPeso"] / event["data"]["invMaxpeso"]) * 100
          }%"></div>`
        );
        $("#weightBarRight").html(
          `<div class="weightBar" style="width: ${
            ((event["data"]["chestPeso"] / event["data"]["chestMaxpeso"]) * 100)
          }%"></div>`
        );
        break;
    }
  ...
  document.onkeyup = (data) => {
    if (data["key"] === "Escape") {
      $.post("http://groupmanager_chest/invClose");
      $(".invRight").html("");
      $(".invLeft").html("");
    }
  };
...
const updateDrag = () => {
  ...
  $(".empty").droppable({
    ...
    drop: function (event, ui) {
      ...
      if (shiftPressed) amount = itemAmount;
      ...
      if (amount == itemAmount) {
        ...
        ui.draggable
          .children(".top")
          .children(".itemAmount")
          .html(formatarNumero(ui.draggable.data("amount")) + "x");
        ui.draggable
          .children(".top")
          .children(".itemWeight")
          .html(newWeightOldItem);

        $(clone1)
          .children(".top")
          .children(".itemAmount")
          .html(formatarNumero(clone1.data("amount")) + "x");
        $(clone1)
          .children(".top")
          .children(".itemWeight")
          .html(newWeightClone1);
      }
      ...
      if (origin === "invLeft" && tInv === "invLeft") {
        ...
        $.post(
          "http://groupmanager_chest/takeItem",
          JSON.stringify({
            item: itemData.key,
            slot: itemData.slot,
            target: target,
        ...
        $.post(
          "http://groupmanager_chest/storeItem",
          JSON.stringify({
            item: itemData.key,
            slot: itemData.slot,
            target: target,
        ...
        $.post(
          "http://groupmanager_chest/updateChest",
          JSON.stringify({
            item: itemData.key,
            slot: itemData.slot,
            target: target,
    ...
    drop: function (event, ui) {
      ...
      if (ui.draggable.data("item-key") == $(this).data("item-key")) {
        ...
        $(this)
          .children(".top")
          .children(".itemAmount")
          .html(formatarNumero(newSlotAmount) + "x");
        $(this)
          .children(".top")
          .children(".itemWeight")
          .html(newSlotWeight.toFixed(2));

        if (amount == itemAmount) {
          ui.draggable.replaceWith(
            `<div class="item empty" data-slot="${ui.draggable.data(
              "slot"
            )}"></div>`
          );
        } else {
          ...
          let newMovedWeight =
            parseFloat(ui.draggable.data("peso")) * newMovedAmount;

          ui.draggable.data("amount", newMovedAmount);
          ui.draggable
            .children(".top")
            .children(".itemAmount")
            .html(formatarNumero(newMovedAmount) + "x");
          ui.draggable
            .children(".top")
            .children(".itemWeight")
            .html(newMovedWeight.toFixed(2));
        }
      } else {
      ...
      $(this).tooltip({
        content: `<item>${name}</item>${
          description !== "undefined"
            ...
            : ""
        }<br><legenda>Tipo: <r>${type}</r> <s>|</s> Máximo: <r>${
          max !== "undefined" ? max : "S/L"
        }</r></legenda>`,
        position: {
          my: "center top+10",
          at: "center bottom",
...
const requestChest = () => {
  $.post(
    "http://groupmanager_chest/requestChest",
    JSON.stringify({}),
    (data) => {
      $("#weightTextLeft").html(
        `${data["invPeso"].toFixed(2)} / ${data["invMaxpeso"].toFixed(2) + "KG"}`
      );
      $("#weightTextRight").html(
        `${data["chestPeso"].toFixed(2)} / ${data["chestMaxpeso"].toFixed(
          2
        ) + "KG" }`
      );
      $("#backend_info").html(`
      ...
      $("#weightBarLeft").html(
        `<div class="weightBar" style="width: ${
          (data["invPeso"] / data["invMaxpeso"]) * 100
        }%"></div>`
      );
      $("#weightBarRight").html(
        `<div class="weightBar" style="width: ${
          (data["chestPeso"] / data["chestMaxpeso"]) * 100
        }%"></div>`
      );
      ...
      for (let x = 1; x <= data["invMaxpeso"]; x++) {
        ...
        if (data.myInventory[slot] !== undefined) {
          ...
          const newDurability =
            (maxDurability - v["durability"]) / maxDurability;
          var actualPercent = newDurability * 100;

          if (actualPercent <= 1) actualPercent = 1;

          const item = `<div class="item populated" title="" data-max="${
            v["max"]
          }" data-type="${v["type"]}" data-description="${
            v["desc"]
          }" style="background-image: url('nui://inventory/web-side/images/${
            v.index
          }.png'); background-position: center; background-repeat: no-repeat;" data-amount="${
            v.amount
          }" data-peso="${v.peso}" data-item-key="${v.key}" data-name-key="${
            v.name
          }" data-slot="${slot}">
					<div class="top">
          <div class="itemAmount">${formatarNumero(v.amount)}x</div>
					...
					<div class="durability" style="width: ${
            actualPercent == 1 ? "100" : actualPercent
          }%; background: ${
            actualPercent == 1 ? "#fc5858" : colorPicker(actualPercent)
          };"></div>
