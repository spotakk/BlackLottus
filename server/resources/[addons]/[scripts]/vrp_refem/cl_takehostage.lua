...
function takeHostage()
	...
	if not canTakeHostage then 
		TriggerEvent("Notify","imporant","Atenção","Você precisa de uma pistola com munição para fazer um refém!.","amarelo",30000)
	end
	...
	if not holdingHostageInProgress and canTakeHostage then		
		...
		if closestPlayer ~= -1 and closestPlayer ~= nil then
			...
			TriggerEvent("Notify","important","Atenção","Ninguém por perto para fazer de réfem.","amarelo",3000)
		end 
	end
