...
if Config.FavKeybindEnabled then
    ...
    function FavKeybind()
        if not IsPauseMenuActive() and not LocalPlayer["state"]["Buttons"] and not LocalPlayer["state"]["Commands"] and not LocalPlayer["state"]["Handcuff"] and GetEntityHealth(PlayerPedId()) > 100 and not LocalPlayer["state"]["Phone"] and not LocalPlayer["state"]["Cancel"] and not IsPedReloading(PlayerPedId()) then
    ...
    if Config.SharedEmotesEnabled then
        sharemenu.OnItemSelect = function(sender, item, index)
            if ShareTable[index] ~= 'none' then
                ...
                if (distance ~= -1 and distance < 3) then
                    ...
                    TriggerServerEvent("ServerEmoteReque", GetPlayerServerId(target), ShareTable[index])
        ...
        shareddancemenu.OnItemSelect = function(sender, item, index)
            ...
            if (distance ~= -1 and distance < 3) then
                ...
                TriggerServerEvent("ServerEmoteReque", GetPlayerServerId(target), DanceTable[index], 'Dances')
                SimpleNotify(Config.Languages[lang]['sentrequestto'] .. GetPlayerName(target))
            else
                SimpleNotify(Config.Languages[lang]['nobodyclose'])
            end
                ...
                searchMenu.OnItemSelect = function(sender, item, index)
                    ...
                    elseif data.table == "Shared" then
                        ...
                        if (distance ~= -1 and distance < 3) then
                            ...
                            TriggerServerEvent("ServerEmoteReque", GetPlayerServerId(target), data.name)
                            SimpleNotify(Config.Languages[lang]['sentrequestto'] .. GetPlayerName(target))
                        else
                            SimpleNotify(Config.Languages[lang]['nobodyclose'])
                        end
                    ...
                    if #sharedDanceMenu.Items > 0 then
                        ...
                        sharedDanceMenu.OnItemSelect = function(sender, item, index)
                            ...
                            if (distance ~= -1 and distance < 3) then
                                ...
                                TriggerServerEvent("ServerEmoteReque", GetPlayerServerId(target), data.name, 'Dances')
                                SimpleNotify(Config.Languages[lang]['sentrequestto'] .. GetPlayerName(target))
                            else
                                SimpleNotify(Config.Languages[lang]['nobodyclose'])
                            end
...
function OpenEmoteMenu()
    ...
    if _menuPool:IsAnyMenuOpen() then
        ...
        LocalPlayer.state:set('onMenu', true, true)
...
function ProcessMenu()
    ...
    LocalPlayer.state:set('onMenu', false, true)
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsPauseMenuActive() or LocalPlayer["state"]["Buttons"] or LocalPlayer["state"]["Commands"] or LocalPlayer["state"]["Handcuff"] or LocalPlayer["state"]["Phone"] or LocalPlayer["state"]["Cancel"] or IsPedReloading(ped) then
             -- cancel emote, destroy props and close menu
             if IsInAnimation then
                EmoteCancel()
            end
