        ...
        if #args > 0 then
            ...
            if (distance ~= -1 and distance < 3) then
                if RP.Shared[emotename] ~= nil then
                    ...
                    TriggerServerEvent("ServerEmoteReque", GetPlayerServerId(target), emotename)
...
RegisterNetEvent("SyncCancelEmo")
AddEventHandler("SyncCancelEmo", function(player)
...
function CancelSharedEmote(ply)
    if targetPlayerId then
        TriggerServerEvent("ServerEmoteCan", targetPlayerId)
        targetPlayerId = nil
    end
end
...
AddEventHandler("ClientEmoteRequestReceive", function(emotename, etype, target)
    ...
    while isRequestAnim do
        ...
        if IsControlJustPressed(1, 246) then
            ...
            TriggerServerEvent("ServerValidEmo", target, requestedemote, otheremote)
