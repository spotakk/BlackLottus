...
Citizen.CreateThread(function()
    TriggerEvent('chat:addSuggestion', '/ex', 'Play an emote',
...
RegisterCommand('ex', function(source, args, raw) EmoteCommandStart(source, args, raw) end, false)
...
if Config.MenuKeybindEnabled then
    ...
    RegisterKeyMapping("emotemenu", "Open EMOTES menu", "keyboard", "f5")
...
RegisterNetEvent('emotes:deleteObjects', function()
    DestroyAllProps()
end)
...
function AddPropToPlayer(prop1, bone, off1, off2, off3, rot1, rot2, rot3, textureVariation)
    ...
    local isSpawned, netId = vRPS.CreateObject((prop1), x, y, z + 0.2)

    if isSpawned then
        prop = LoadNetwork(netId)
